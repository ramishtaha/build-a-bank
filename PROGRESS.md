...
- **Step:** 10 of 67 — *Relational Databases Up Close* — ✅ **COMPLETE & VERIFIED** (🔴 Full tier)
- **Last verified tag:** `step-10-end` (== `step-11-start`) — `./mvnw -pl services/cif -am verify` → BUILD SUCCESS, **21 cif tests** (10 + 11 new raw-JDBC labs). Query plans proven (Seq Scan → Bitmap Index Scan → Index Only Scan `Heap Fetches: 0`); isolation anomalies (no dirty read; non-repeatable @RC; stable @RR; phantom @RC not @RR); **write skew** broken at REPEATABLE READ (sum −100) and rejected at SERIALIZABLE (SQLSTATE 40001); HikariCP exhaustion (~506 ms timeout); partition pruning (only `txn_2026_02`); online DDL (CIC 25001-in-txn / works in autocommit; fast default). §12.3 mutation (SERIALIZABLE→RR) caught + reverted; smoke.sh PASSED. ADR-0004 records the raw-JDBC-on-shared-Testcontainers lab approach.
- **Next action:** **Step 11 — Concurrency & Thread Safety in Java** 🟣: the Java Memory Model (happens-before, visibility, reordering, `volatile`), mutual exclusion, atomics/CAS (`LongAdder`), `java.util.concurrent` (executors & pool sizing, `CompletableFuture`, `ConcurrentHashMap`, latches/semaphores), safe publication/immutability/confinement, classic bugs (races, deadlock, double-checked locking), **virtual threads** (what they change & don't), structured concurrency; **testing concurrency with JCStress**. Buildable: a shared-balance race shown **failing** (JCStress and/or a stress test) then fixed and proven; likely a new `concurrency-lab` test surface (decide module placement — reuse a service test tree or a small lab module; record ADR if architectural). Verify JCStress resolves on JDK 25 / Boot 4 (add to VERSIONS.md). Keep `step-11-end == step-12-start`. (No Docker strictly required; pure-JVM.)
...
- ✅ **Steps 2–10** — all tagged `step-NN-end` and green (see the Verification ledger below). Phase A complete (Steps 1–7: language, web, JVM, Spring Core/Boot/AOP). Phase B in progress: Step 8 (CIF + JPA + Flyway + Testcontainers), Step 9 (Hibernate perf/correctness), Step 10 (relational DB up close — query plans, MVCC/isolation, write skew, pool, partitioning, online DDL). Modules so far: `services/hello`, `services/cif`, plus per-phase learning modules (`java-basics`, `net`, `jvm`, `spring-lab`) per ADR-0003.

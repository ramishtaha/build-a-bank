...
| **C** | [🛠️ Build](#build) | 14 sub-steps: the domain → the ports → the use case → the adapter ring → tests → harness | ~7.5 h |
| **D** | [🔬 Prove](#prove) | the Verification Log — unchanged tests still green (behaviour preserved); §12.3 mutation | ~45 min |
...
≈ 12 hours rarely fits one sitting. Five sittings of ~2–3 h, each ending at a real save point:

| Sitting | Covers | ~Time | Ends at (save point) |
|---|---|---|---|
| **S1 · Draw the hexagon** | A Orient + B Understand + the B→C before/after tree | ~2.5 h | ✋ end of the B→C bridge — you can redraw the AFTER tree from memory |
| **S2 · Baseline + domain + first port** | Sub-steps 0–3 (green baseline · `domain/TransferEvent` · `domain/Notification` · `ProcessedEventStore` → `application/port/out`) | ~2 h | ✋ sub-step 3 — the relocated port sits in `application/port/out/` |
| **S3 · Finish the core** | Sub-steps 4–6 (`NotificationPublisher` · `NotifyOnTransfer` · the `NotificationService` use case) | ~2 h | ✋ sub-step 6 — the core is complete and closed |
| **S4 · The adapter ring** | Sub-steps 7–11 (persistence → push *(tree goes red — on purpose)* → messaging → the thin consumer → web + cleanup) | ~3 h | ✋ sub-step 11 — the 13-file hexagon compiles GREEN |
| **S5 · Prove + wrap** | Sub-steps 12–13 (five test-import fixes → 7/7 · Makefile/smoke.sh/requests.http/ADR-0017 + the §12.3 mutation) + D Prove + E Apply + F Review | ~2.5 h | ✋ sub-step 13 → Definition of Done → `step-26-end` tagged |

**Optional routes:** the ⏭️ skip-test (5 min) may let you skim the whole Build; the four 🚀 Go Deeper asides are +~10 min each; the 🧪 little experiments are +~15 min; the two 🎯 Stretch challenges (Redis store · second publisher) are +~45–60 min each — fit them into S5 or a bonus sitting.

✋ **Stopping here?** You have the step's map (cheat card + effort budget + this plan). Next: B · 🧠 Understand (the Big Idea); first action: jump to [#understand](#understand) and read the three rings against the Cheat Card diagram.
...
❓ **Knowledge-check:** `NotifyOnTransfer` and `NotificationPublisher` are both interfaces defined by the
application — which is inbound and which is outbound, and who calls/implements each?
<details><summary>answer</summary><code>NotifyOnTransfer</code> is <strong>inbound</strong>: the app
<em>offers</em> it — implemented by <code>NotificationService</code>, called by the driving Kafka listener.
<code>NotificationPublisher</code> is <strong>outbound</strong>: the app <em>needs</em> it — declared by the
core, implemented by the driven SSE adapter. Same word "port," opposite implementation direction.</details>
...
Next: Sub-step 0 (the green baseline); first action: `./mvnw -pl services/notification test` with Docker running.
...
### Sub-step 0 of 13 — Establish the green baseline · ⏱ ~15 min 🧭 *(you are here: **baseline** → core → ring → tests → harness)*
...
✋ **Stopping here?** You have a proven-green 7/7 baseline on `step-26-start`. Next: Sub-step 1
(`domain/TransferEvent`); first action: create
...
### Sub-step 1 of 13 — `domain/TransferEvent`: the first pure-domain citizen · ⏱ ~30 min 🧭 *(baseline ✅ → **domain** → ports → use case → ring → tests → harness)*
...
### Sub-step 2 of 13 — `domain/Notification`: the second value object (factory included) · ⏱ ~25 min 🧭 *(baseline ✅ → domain ½ → **domain** → ports → use case → ring → tests → harness)*
...
### Sub-step 3 of 13 — `application/port/out/ProcessedEventStore`: the Step-25 port finds its true home · ⏱ ~25 min 🧭 *(domain ✅ → **ports** → use case → ring → tests → harness)*
...
✋ **Stopping here (end of S2)?** You have the complete domain ring plus one relocated port in
`application/port/out/`. Next: Sub-step 4 (the `NotificationPublisher` port); first action: create
...
### Sub-step 4 of 13 — `application/port/out/NotificationPublisher`: the second outbound port (NEW) · ⏱ ~20 min 🧭 *(domain ✅ → ports ⅓ → **ports** → use case → ring → tests → harness)*
...
### Sub-step 5 of 13 — `application/port/in/NotifyOnTransfer`: the inbound port (NEW) · ⏱ ~25 min 🧭 *(domain ✅ → ports ⅔ → **ports** → use case → ring → tests → harness)*
...
### Sub-step 6 of 13 — `application/NotificationService`: the use case (the heart of the step) · ⏱ ~45 min 🧭 *(domain ✅ → ports ✅ → **use case** → ring → tests → harness)*
...
✋ **Stopping here (end of S3)?** You have the complete, closed core — 2 domain records, 3 ports, 1 use case —
and a module that still compiles. Next: Sub-step 7 (the first driven adapter); first action:
`mkdir -p services/notification/src/main/java/com/buildabank/notification/adapter/out/persistence`, then the
`git mv` at the top of sub-step 7.
...
### Sub-step 7 of 13 — `adapter/out/persistence/InMemoryProcessedEventStore`: the first driven adapter · ⏱ ~30 min 🧭 *(core ✅ → **ring: persistence** → push → messaging → consumer → web → tests → harness)*
...
### Sub-step 8 of 13 — `adapter/out/push/SseHub`: the push adapter (and the tree goes red) · ⏱ ~40 min 🧭 *(core ✅ → persistence ✅ → **push** → messaging → consumer → web → tests → harness)*
...
✋ **Stopping here?** You have a deliberately red tree — the flat consumer and controller still name the moved
`SseHub`; the compiler's error list is your to-do list. Next: Sub-step 9 (parser + DLT config); first action:
`mkdir -p services/notification/src/main/java/com/buildabank/notification/adapter/in/messaging`, then the two
`git mv` commands.
...
### Sub-step 9 of 13 — `adapter/in/messaging/`: the parser and the DLT config move in · ⏱ ~25 min 🧭 *(core ✅ → out-adapters ✅ → **messaging** → consumer → web → tests → harness)*
...
❓ **Knowledge-check:** which ring may import Jackson, and why? <details><summary>answer</summary>Only the
**adapter** ring (here: <code>adapter/in/messaging/TransferEventParser</code>). Jackson is a wire-format
detail; the core speaks <code>TransferEvent</code>, not JSON — which is also why a poison payload dies in the
adapter (→ DLT, Step 21) and never reaches the core.</details>
...
### Sub-step 10 of 13 — The thin consumer: a driving adapter that *drives the port* · ⏱ ~45 min 🧭 *(core ✅ → out-adapters ✅ → messaging ✅ → **consumer** → web → tests → harness)*
...
### Sub-step 11 of 13 — `adapter/in/web/NotificationController` + delete the flat leftovers → GREEN · ⏱ ~40 min 🧭 *(… consumer ✅ → **web + cleanup** → tests → harness)*
...
✋ **Stopping here (end of S4)?** You have a green 13-file hexagon (flat package holds only
`NotificationApplication.java`). Next: Sub-step 12 (the proof — test imports); first action: add the import
lines to the five files under `services/notification/src/test/java/com/buildabank/notification/`.
...
### Sub-step 12 of 13 — Fix the five test imports, run the proof · ⏱ ~40 min 🧭 *(… web ✅ → **tests — the proof** → harness)*
...
### Sub-step 13 of 13 — The play/verify harness: Makefile, smoke.sh, requests.http, ADR · ⏱ ~45 min 🧭 *(… tests ✅ → **harness** — last stop)*
...
✋ **Stopping here?** You have the harness green (`make play-26`, `smoke.sh`, ADR-0017) and the mutation seen
failing then reverted. Next: 🎮 Play With It, then 🏁 the Definition of Done; first action: open
`steps/step-26/requests.http` (or tick the ✅ boxes in 🏁 The Finished Result).
...
❓ **Knowledge-check:** in the diagram's duplicate branch, which ring makes the "don't publish" decision, and
through which port does the answer travel back to the `applied` counter? <details><summary>answer</summary>The
**application** ring — <code>NotificationService.handle</code> sees <code>markIfNew</code> return false and
skips publishing; the <code>false</code> travels back through the inbound port <code>NotifyOnTransfer</code>,
so the messaging adapter's <code>applied</code> counter stays honest.</details>
...
🧪 **Little experiments (+~15 min — each one teaches a boundary):**
...
✋ **Stopping here (end of the Build)?** You have every Definition-of-Done box ticked. Next: D · 🔬 Prove It
Works; first action: compare your own `./mvnw -pl services/notification test` output against §1 of the
Verification Log.
...
✋ **Stopping here after the proof?** You have a verified, behaviour-preserved hexagon (Verification Log §1–§5
checked against your run). Next: E · 🎓 Apply; first action: the first Quick challenge in 🏋️ Your Turn — write
`NotificationServiceTest` mocking both outbound ports.

## 🚀 Go Deeper (Optional)

<details><summary>Hexagonal vs Clean vs Onion — one idea, three vocabularies (+~10 min)</summary>
...
<details><summary>Where does Spring fit if the core is "framework-free"? (+~10 min)</summary>
...
<details><summary>Why the test classes didn't move into layer packages (+~10 min)</summary>
...
<details><summary>Ports across the bank: you've been using this idea all along (+~10 min)</summary>
...
- **Quick (+~15 min):** write the `NotificationService` unit test this structure unlocked — mock both outbound ports (no
...
- **Quick (+~10 min):** from memory, list the imports allowed in each ring (domain → ?; application → ?; adapters → ?),
...
- 🎯 **Stretch (+~45–60 min) — the durable dedup adapter:** implement `RedisProcessedEventStore` in
...
- 🎯 **Stretch (+~45–60 min) — a second publisher adapter:** a `LoggingNotificationPublisher` (or webhook one, reusing
...
✋ **Stopping here (end of the Apply half)?** You have the challenges done or parked. Next: F · 🏆 Review;
first action: answer the five 🧠 Test Yourself questions in the Recap without peeking.

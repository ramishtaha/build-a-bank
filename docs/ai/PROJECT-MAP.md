...
| auth | `services/auth` | 8083 | — (in-memory users) | Step 16 | SecurityFilterChain + HS256 JWT + BCrypt (16); RS256 + public `/oauth2/jwks` + @PreAuthorize (17) |
...
| gateway | `gateway` | 8080 | — | Step 15 | Spring Cloud Gateway Server WebMVC (servlet), StripPrefix routes + @HttpExchange CifClient (15); fronts auth + deny-by-default CORS for SPA (29); fronts notification SSE (30) |
...
| frontend (npm, not Maven) | `frontend/` | 5173 (Vite dev) | — | Step 29 | React 19 + TS + Vite; typed API client → gateway, AuthContext JWT, ProtectedRoute (29); TanStack Query + RHF/Zod transfer form + SSE EventSource (30) |
...
- Tag chain: `step-NN-start == step-(NN-1)-end`; every verified step tagged `step-NN-end`. Latest verified tag: `step-30-end` (== `step-31-start`). Step 31 in progress.
...
- 0015 spring-batch-eod-and-phase-d-capstone · 0016 solid-refactor-notification-ports · 0017 hexagonal-notification-service · 0018 archunit-and-spring-modulith · 0019 testing-quality-mastery-and-custom-starter · 0020 frontend-foundations-and-gateway-front-door · 0021 frontend-data-forms-live-updates
...
| `/notifications/**` | notification :8084 | StripPrefix=1 (SSE stream; servlet gateway may buffer) |

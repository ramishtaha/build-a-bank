...
**The bar:** match the depth and shape of the gold-standard worked example — see `steps/step-08/lesson.md` or `steps/step-12/lesson.md` (both full micro-anatomy, verified) for in-repo exemplars, or master prompt §8.1 for the canonical one. **Anti-anchoring rule:** the most recent lessons are NOT the style reference — if they drifted, match the spec and the strongest exemplar, never the neighbors. Sanity floor: a typical build-heavy lesson lands ~1,000–2,000 lines with ~80–120 fenced blocks; a few hundred lines means the build was gutted.
